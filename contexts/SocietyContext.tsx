...
import {
  Society,
  SocietyContext as SocietyContextType,
  AdminUser,
} from '@/types/admin';
...
const SocietyContext = createContext<ExtendedSocietyContextType | undefined>(
  undefined,
);
...
export const SocietyProvider: React.FC<SocietyProviderProps> = ({
  ...
  const [societyData, setSocietyDataState] = useState<Map<string, any>>(
    new Map(),
  );
  ...
  const initializeSocieties = async () => {
    ...
    try {
      ...
      const defaultSociety =
        societies.find((s) => s.id === lastSelectedSociety) ||
  ...
  const loadAccessibleSocieties = async (
    admin: AdminUser,
  ): Promise<Society[]> => {
    // Mock implementation - replace with actual API call
    return new Promise((resolve) => {
      setTimeout(() => {
        const mockSocieties: Society[] = admin.societies.map(
          (access, index) => ({
            id: access.societyId,
  ...
  const validateSocietyAccess = (societyId: string): boolean => {
    ...
    return adminUser.societies.some(
      (access) => access.societyId === societyId && access.isActive,
    );
  };
  ...
  const refreshSocieties = async (): Promise<void> => {
    ...
    try {
      ...
      if (currentSociety) {
        const updatedCurrentSociety = societies.find(
          (s) => s.id === currentSociety.id,
        );
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          ...
          results: accessibleSocietyIds.reduce(
            (acc, societyId) => {
              acc[societyId] = {
  ...
  const filterNotificationsForSociety = (
    notifications: any[],
    societyId: string,
  ): any[] => {
  ...
  const getLastSelectedSociety = async (
    userId: string,
  ): Promise<string | null> => {
    try {
  ...
  const saveLastSelectedSociety = async (
    userId: string,
    societyId: string,
  ): Promise<void> => {
    try {
  ...
  const logSocietySwitch = async (
    switchRecord: SocietySwitchRecord,
  ): Promise<void> => {
  ...
  return (
    <SocietyContext.Provider value={value}>{children}</SocietyContext.Provider>
  );
};
