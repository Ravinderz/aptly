...
import { StatWidget, QuickActionWidget, ProgressWidget } from './DashboardWidgets';
  ...
  const handleBulkBillGeneration = async () => {
    if (!checkPermission('billing', 'bulk_action')) {
      showAlert({
        ...
        message: 'You do not have permission to perform bulk billing operations',
    ...
    try {
      ...
      showAlert({
        ...
        message: error instanceof Error ? error.message : 'Failed to generate bulk bills',
        primaryAction: { label: 'OK', onPress: () => {} }
      });
    }
  };
          ...
          {(['week', 'month', 'quarter'] as const).map((period) => (
            ...
            >
              <Text className={`text-center font-medium capitalize ${
                selectedTimeframe === period ? 'text-white' : 'text-text-secondary'
              }`}>
                {period}
              </Text>
            </TouchableOpacity>
        ...
        <View className="px-6 pb-6">
          ...
          <View className="space-y-3">
            {societySummaries.map((summary) => (
              <View key={summary.societyId} className="bg-surface rounded-xl p-4 border border-divider">
                <View className="flex-row items-center justify-between mb-3">
                  <Text className="text-body-large font-semibold text-text-primary">
                    {summary.societyName}
                  </Text>
                  <View className={`px-2 py-1 rounded-full ${
                    summary.collectionRate >= 80 ? 'bg-green-100' : 
                    summary.collectionRate >= 60 ? 'bg-orange-100' : 'bg-red-100'
                  }`}>
                    <Text className={`text-label-small font-medium ${
                      summary.collectionRate >= 80 ? 'text-green-600' : 
                      summary.collectionRate >= 60 ? 'text-orange-600' : 'text-red-600'
                    }`}>
                ...
                <View className="flex-row justify-between items-center">
                  <View className="flex-1">
                    <Text className="text-label-medium text-text-secondary">Total Bills</Text>
                    <Text className="text-body-medium font-semibold text-text-primary">
                      {summary.totalBills}
                    </Text>
                  </View>
                  <View className="flex-1">
                    <Text className="text-label-medium text-text-secondary">Pending</Text>
                    <Text className="text-body-medium font-semibold text-orange-600">
                      {summary.pendingBills}
                    </Text>
                  </View>
                  <View className="flex-1">
                    <Text className="text-label-medium text-text-secondary">Amount</Text>
              ...
              return (
                <View key={action.id} className="bg-surface rounded-xl p-4 border border-divider">
                  <View className="flex-row items-start justify-between mb-2">
                    <View className="flex-1 mr-3">
                      ...
                      <View className="flex-row items-center">
                        ...
                        <Text className={`text-label-medium font-medium ml-2 ${getStatusColor(action.status)}`}>
                  ...
                  <Text className="text-label-medium text-text-secondary">
                    ...
                    {action.completedDate ? ` Completed ${new Date(action.completedDate).toLocaleDateString()}` : 
                     action.scheduledDate ? ` Scheduled for ${new Date(action.scheduledDate).toLocaleDateString()}` : 
                     ' In Progress'}
                  </Text>
                </View>
              );
