...
const CommentInput: React.FC<CommentInputProps> = ({
  ...
  const loadUsers = async () => {
    try {
      const mockUsers: User[] = [
        { id: 'user1', name: 'Ravinder Singh', flatNumber: 'A-201', role: 'resident' },
  ...
  const canSubmit = content.trim().length >= 3 && characterCount <= MAX_COMMENT_LENGTH;
    ...
    <View className="border-t border-divider bg-surface px-4 py-3">
      <View className="flex-row items-end gap-3">
        <View className="flex-1">
          ...
          />
          {content.length > 0 && (
            <Text className={`text-xs mt-1 text-right ${isNearLimit ? 'text-warning' : 'text-text-secondary'}`}>
