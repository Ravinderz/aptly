import { CalendarDays, Check, Clock, Eye, MessageSquare, X } from "lucide-react-native";
...
const VisitorListItem: React.FC<VisitorListItemProps> = ({
  ...
  return (
    <View className="bg-surface border border-divider rounded-xl p-4 mb-3">
        ...
        <View className="flex-row mt-4 pt-3 border-t border-divider/50">
          ...
          {(status === "Approved" || status === "Pre-approved" || status === "Pre-Approved") && (
            <>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <CalendarDays size={16} className="text-text-secondary" strokeWidth={1.5} />
          ...
          {type === "past" && (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Eye size={16} className="text-text-secondary" strokeWidth={1.5} />
              <Text className="text-label-large font-medium text-text-secondary ml-2">
                View Details
              </Text>
            </TouchableOpacity>
      ...
      {(status === "Approved" || status === "Pre-approved" || status === "Pre-Approved") && phone && (
        <View className="mt-3 pt-3 border-t border-divider/50">
          <TouchableOpacity 
            className="flex-row items-center py-2"
            onPress={() => {/* TODO: Call functionality */}}
            activeOpacity={0.7}
          >
