...
const VEHICLE_TYPES = [
  { key: 'car', label: 'Car', icon: <Car size={24} color="#6366f1" /> },
  {
    key: 'bike',
    label: 'Bike/Scooter',
    icon: <Bike size={24} color="#6366f1" />,
  },
  {
    key: 'bicycle',
    label: 'Bicycle',
...
export default function VehicleForm({
  ...
  const validateRegistrationNumber = (regNumber: string): ValidationResult => {
    ...
    if (!indianRegPattern.test(regNumber.toUpperCase())) {
      return {
        isValid: false,
        error:
          'Please enter a valid Indian registration number (e.g., DL-01-AB-1234)',
      };
    }
  ...
  const validateParkingSlot = (slot: string): ValidationResult => {
    ...
    if (slot.length > 10) {
      return {
        isValid: false,
        error: 'Parking slot must be less than 10 characters',
      };
    }
    ...
    <ScrollView
      ...
      <Card className="space-y-6">
        ...
        <View>
          ...
          {errors.type && (
            <Text className="text-error text-sm mt-2">{errors.type}</Text>
          )}
        </View>
        ...
        <View>
          ...
          <TouchableOpacity
            ...
            <Text
              className={
                formData.make ? 'text-text-primary' : 'text-text-secondary'
              }>
          ...
          {errors.make && (
            <Text className="text-error text-sm mt-2">{errors.make}</Text>
          )}
        </View>
        ...
        <View>
          ...
          {errors.model && (
            <Text className="text-error text-sm mt-2">{errors.model}</Text>
          )}
        </View>
        ...
        <View>
          ...
          {errors.color && (
            <Text className="text-error text-sm mt-2">{errors.color}</Text>
          )}
        </View>
        ...
        <View className="bg-background rounded-xl p-4 border border-divider">
          <View className="flex-row items-center justify-between">
            ...
            <TouchableOpacity
              onPress={() =>
                handleInputChange('isPrimary', !formData.isPrimary)
              }
