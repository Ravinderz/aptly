...
import { showErrorAlert, showDeleteConfirmAlert, showConfirmAlert } from '@/utils/alert';
...
const ImagePicker: React.FC<ImagePickerProps> = ({
  ...
  const openCamera = async () => {
    try {
      ...
      if (status !== 'granted') {
        showErrorAlert('Permission Required', 'Camera permission is required to take photos.');
        return;
      }
    ...
    >
      {uploading ? (
        <>
          ...
          <Text className="text-text-secondary mt-3 text-center">Processing image...</Text>
        </>
      ) : (
        <>
