...
const mockVendorDetails = {
  '1': {
    ...
    responseTime: '< 30 min',
    description:
      'Expert in bathroom renovations and water system repairs. Available for emergency services 24/7. Licensed plumber with insurance coverage.',
    ...
    recentWork: [
      {
        date: '2024-01-15',
        description: 'Complete bathroom renovation in B-402',
        rating: 5,
      },
      {
        date: '2024-01-10',
        description: 'Pipeline repair in common area',
        rating: 5,
      },
      {
        date: '2024-01-05',
        description: 'Water heater installation in A-301',
        rating: 4,
      },
    ],
  },
...
const mockReviews = [
  {
    ...
    date: '2024-01-15',
    comment:
      'Excellent work! Fixed our bathroom leak quickly and professionally. Highly recommended.',
    service: 'Pipe Repair',
    helpful: 12,
    images: [],
  },
  {
    ...
    date: '2024-01-10',
    comment:
      'Very punctual and skilled. Completed the water tank cleaning on time. Fair pricing.',
  ...
  },
  {
    ...
    date: '2024-01-05',
    comment:
      'Good service but took a bit longer than expected. Quality of work was good.',
...
export default function VendorProfile() {
  ...
  const { category, vendorId } = useLocalSearchParams<{
    category: string;
    vendorId: string;
  }>();
  ...
  const handleWhatsApp = () => {
    ...
    Linking.openURL(
      `whatsapp://send?phone=${vendor.phone.replace(/\s/g, '')}&text=${encodeURIComponent(message)}`,
    );
  };
    ...
    <View>
      ...
      <View className="bg-surface rounded-xl p-4 mb-4 border border-divider">
        ...
        <View className="flex-row flex-wrap gap-2">
          {vendor.certifications.map((cert, index) => (
            <View
              key={index}
              className="bg-secondary/10 rounded-full px-3 py-2 flex-row items-center">
      ...
      <View className="bg-surface rounded-xl p-4 mb-4 border border-divider">
        ...
        {vendor.recentWork.map((work, index) => (
          <View
            key={index}
            className="flex-row items-center justify-between py-2 border-b border-divider last:border-b-0">
    ...
    <View>
      ...
      {vendor.services.map((service, index) => (
        <ServiceCard
          ...
          service={service}
          onSelect={() =>
            showAlert('Service Selected', `You selected: ${service.name}`)
          }
        />
      ))}
    </View>
          ...
          <View className="flex-row items-center justify-between mb-4">
            <View className="flex-1">
              <View className="flex-row items-center mb-2">
                ...
                {vendor.verified && <Shield size={20} color="#4CAF50" />}
              </View>
