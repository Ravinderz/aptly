...
import { ArrowLeft, Zap, User, AlertCircle, CreditCard, MapPin } from 'lucide-react-native';
...
export default function ElectricityRecharge() {
  ...
  const providers: ElectricityProvider[] = [
    { id: 'bescom', name: 'BESCOM', state: 'Karnataka', logo: '⚡', color: '#1976D2' },
    { id: 'msedcl', name: 'MSEDCL', state: 'Maharashtra', logo: '🏢', color: '#4CAF50' },
    { id: 'tsspdcl', name: 'TSSPDCL', state: 'Telangana', logo: '⚡', color: '#FF9800' },
    { id: 'kseb', name: 'KSEB', state: 'Kerala', logo: '🌴', color: '#2E7D32' },
    { id: 'tneb', name: 'TNEB', state: 'Tamil Nadu', logo: '🏛️', color: '#D32F2F' },
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      <View className="flex-row items-center px-4 py-4 bg-surface border-b border-divider">
        ...
        <View className="flex-row items-center flex-1">
          ...
          <View>
            <Text className="text-headline-medium text-text-primary">Electricity Bill</Text>
            <Text className="text-text-secondary text-body-medium">Pay your electricity bill</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-4 py-6" showsVerticalScrollIndicator={false}>
        ...
        <Card className="mb-6">
          <View className="p-4">
            <Text className="text-headline-medium text-text-primary mb-4">Consumer Details</Text>
            
            <View className="mb-4">
              <Text className="text-text-primary font-medium mb-2">Consumer Number</Text>
            ...
            <Button 
              ...
              className="w-full"
            >
              {isDetecting ? 'Detecting Provider...' : selectedProvider ? 'Provider Detected' : 'Detect Provider'}
            </Button>
          </View>
        </Card>
        ...
        {selectedProvider && (
          <Card className="mb-6">
            <View className="p-4">
              <Text className="text-headline-medium text-text-primary mb-4">Electricity Provider</Text>
        ...
        {selectedProvider && (
          <Card className="mb-6">
            <View className="p-4">
              <Text className="text-headline-medium text-text-primary mb-4">Bill Amount</Text>
              
              <View className="mb-4">
                <View className="flex-row items-center bg-surface border border-divider rounded-xl px-4 py-3">
                  <Text className="text-text-primary text-body-large font-medium mr-2">₹</Text>
                  <TextInput
        ...
        {selectedProvider && (
          <Card className="mb-6">
            <View className="p-4">
              <Text className="text-headline-medium text-text-primary mb-4">Quick Amounts</Text>
