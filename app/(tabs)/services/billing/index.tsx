...
export default function Billing() {
  ...
  const bills = [
    ...
    },
    {
      ...
      status: 'pending',
      type: 'common_area',
      items: [{ description: 'Lobby Lighting Repair', amount: 2800 }],
      splitBetween: 25,
      costPerFlat: 132,
    },
  ];
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'paid':
        return (
          <LucideIcons name="checkmark-circle" size={16} color="#4CAF50" />
        );
      case 'pending':
        return (
          <LucideIcons name="calendar-outline" size={16} color="#FF9800" />
        );
      case 'overdue':
        return <LucideIcons name="alert-circle" size={16} color="#D32F2F" />;
      default:
        return (
  ...
  const filteredBills = bills.filter((bill) => {
    ...
    if (selectedTab === 'pending')
      matchesTab = bill.status === 'pending' || bill.status === 'overdue';
  ...
  const totalPaidThisMonth = bills
    .filter(
      (bill) => bill.status === 'paid' && bill.paidDate?.includes('2024-03'),
    )
    .reduce((sum, bill) => sum + bill.totalAmount, 0);
    ...
    <SafeAreaView className="flex-1 bg-background">
      <ResponsiveContainer
        ...
        <View className="flex-row items-center px-6 py-4 border-b border-divider bg-surface">
          ...
          <TouchableOpacity
            onPress={() =>
              router.push('/(tabs)/services/billing/notifications')
            }
        ...
        <View className="px-6 py-6 bg-surface border-b border-divider">
          <View className="flex-row gap-4 mb-6">
            <View className="flex-1 bg-error/10 rounded-xl p-4 border border-error/20">
              ...
              <Text className="text-error text-display-small font-bold mb-1">
                {formatCurrency(totalPending)}
              </Text>
              <Text className="text-error/70 text-label-large">
                3 bills pending
              </Text>
            </View>
            <View className="flex-1 bg-success/10 rounded-xl p-4 border border-success/20">
              <View className="flex-row items-center mb-3">
                <LucideIcons
                  ...
                  color="#4CAF50"
                />
                <Text className="text-success text-body-medium font-medium ml-2">
                  Paid This Month
                </Text>
              </View>
              <Text className="text-success text-display-small font-bold mb-1">
                {formatCurrency(totalPaidThisMonth)}
              </Text>
              <Text className="text-success/70 text-label-large">
                On time payment
              </Text>
            </View>
          </View>
        ...
        <View className="px-6 py-6 bg-background">
          <View className="flex-row items-center justify-between mb-4">
            ...
            <View className="bg-success/10 rounded-full px-3 py-1">
              <Text className="text-success text-label-large font-bold">
                💰 Earn Cashback
              </Text>
            </View>
          </View>
          ...
          <Text className="text-text-secondary text-body-medium mb-6 leading-5">
            Pay utility bills and recharge services. Earn cashback on every
            transaction!
          </Text>
          ...
          <View className="flex-row flex-wrap gap-4">
            ...
            <TouchableOpacity
              onPress={() =>
                router.push('/(tabs)/services/billing/mobile-recharge')
              }
              ...
              <View className="bg-primary/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="call-outline" size={20} color="#6366f1" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                Mobile Recharge
              </Text>
              <Text className="text-text-secondary text-label-large text-center">
                Prepaid & Postpaid
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              onPress={() =>
                router.push('/(tabs)/services/billing/broadband-recharge')
              }
              className="bg-surface rounded-xl p-4 border border-divider flex-1 min-w-[45%] items-center"
              activeOpacity={0.7}>
              <View className="bg-secondary/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="home-outline" size={20} color="#4CAF50" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                Broadband
              </Text>
            ...
            <TouchableOpacity
              onPress={() =>
                router.push('/(tabs)/services/billing/cylinder-booking')
              }
              className="bg-surface rounded-xl p-4 border border-divider flex-1 min-w-[45%] items-center"
              activeOpacity={0.7}>
              <View className="bg-warning/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="warning" size={20} color="#FF9800" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                Gas Cylinder
              </Text>
            ...
            <TouchableOpacity
              onPress={() =>
                router.push('/(tabs)/services/billing/gas-recharge')
              }
              className="bg-surface rounded-xl p-4 border border-divider flex-1 min-w-[45%] items-center"
              activeOpacity={0.7}>
              <View className="bg-error/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="car-outline" size={20} color="#D32F2F" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                Prepaid Gas
              </Text>
            ...
            <TouchableOpacity
              onPress={() =>
                router.push('/(tabs)/services/billing/dishtv-recharge')
              }
              className="bg-surface rounded-xl p-4 border border-divider flex-1 min-w-[45%] items-center"
              activeOpacity={0.7}>
              <View className="bg-primary/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="home-outline" size={20} color="#6366f1" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                DishTV
              </Text>
            ...
            <TouchableOpacity
              onPress={() =>
                showAlert(
                  'Coming Soon',
                  'Electricity bill payment will be available soon!',
                )
              }
              className="bg-surface rounded-xl p-4 border border-divider flex-1 min-w-[45%] items-center opacity-60"
              activeOpacity={0.7}>
              <View className="bg-warning/10 rounded-full w-12 h-12 items-center justify-center mb-3">
                <LucideIcons name="home-outline" size={20} color="#FF9800" />
              </View>
              <Text className="text-text-primary font-semibold text-body-medium text-center mb-1">
                Electricity
              </Text>
        ...
        <View className="px-6 py-4 bg-background border-t border-divider">
          <View className="flex-row items-center mb-2">
            <LucideIcons name="home-outline" size={20} color="#6366f1" />
            <Text className="text-headline-large font-semibold text-text-primary ml-3">
              Society Bills
            </Text>
          </View>
        ...
        <View className="px-6 py-5 bg-surface border-b border-divider">
          <View className="flex-row items-center bg-background rounded-xl px-4 py-4 mb-5">
            <LucideIcons name="search-outline" size={20} color="#757575" />
            <TextInput
            ...
            <View className="flex-row gap-3">
              {[
                ...
                },
                {
                  key: 'common_area',
                  label: 'Common Area',
                  count: bills.filter((b) => b.type === 'common_area').length,
                },
              ].map((tab) => (
                <TouchableOpacity
                  key={tab.key}
          ...
          <View>
              ...
              <ResponsiveCard
                ...
                <TouchableOpacity
                      ...
                      <View className="flex-1 min-w-0 mr-4">
                        ...
                        {bill.type === 'common_area' && (
                          <ResponsiveText
                            ...
                            numberOfLines={2}>
                            Split between {bill.splitBetween} flats • Your
                            share: {formatCurrency(bill.costPerFlat)}
                          </ResponsiveText>
                        )}
                      </View>
                  ...
                  <ResponsiveRow
                    ...
                    className="mb-4">
                    <View
                      className={`flex-row items-center px-3 py-2 rounded-full ${getStatusBgColor(bill.status)}`}>
                      {getStatusIcon(bill.status)}
                      <ResponsiveText
                        className={`font-medium ml-2 capitalize ${getStatusColor(bill.status)}`}>
                        {bill.status}
                      </ResponsiveText>
                    </View>
                    <View className="items-end flex-shrink-0">
            ...
            {filteredBills.length === 0 && (
              <View className="flex-1 items-center justify-center py-16">
                <Text className="text-text-secondary text-headline-small mb-2">
                  No bills found
                </Text>
            ...
            {selectedTab === 'pending' && filteredBills.length > 0 && (
              <HighlightCard
                ...
                <Text className="text-text-secondary leading-5">
                  All payments are processed through encrypted gateways. Your
