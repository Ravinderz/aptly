...
export default function MaintenanceRequestDetail() {
  ...
  const mockRequest = {
    ...
    description:
      "Kitchen sink is leaking from the bottom. Water is dripping continuously. The issue started yesterday evening and has been getting worse. I've placed a bucket to catch the water but it needs immediate attention.",
    category: 'Plumbing',
    priority: 'high',
    status: 'in_progress',
    date: '2024-03-15',
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      showSuccessAlert(
        'Note Added',
        'Timeline entry has been added successfully',
      );
      setNewNote('');
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              showSuccessAlert(
                'Request Completed',
                'Maintenance request has been marked as completed',
              );
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      <ScrollView
        ...
        <View className="flex-row items-center justify-between mb-6">
          <View
            className={`px-4 py-2 rounded-full ${getStatusBgColor(mockRequest.status)}`}>
            <Text
              className={`font-medium capitalize ${getStatusColor(mockRequest.status)}`}>
              {mockRequest.status.replace('_', ' ')}
            </Text>
          </View>
          <View
            className={`px-3 py-1 rounded-full border ${getPriorityColor(mockRequest.priority)}`}>
        ...
        <Card className="mb-6">
          <View className="flex-row items-start mb-4">
            <Text className="text-2xl mr-3">
              {getCategoryIcon(mockRequest.category)}
            </Text>
            <View className="flex-1">
              ...
              <View className="flex-row items-center flex-1">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-text-secondary text-sm">
                    {mockRequest.category} Specialist
                  </Text>
                </View>
              </View>
              <TouchableOpacity
        ...
        <View className="space-y-3">
          ...
          {mockRequest.status === 'pending' && (
            <Button
              ...
              onPress={() => {
                setNewNote(
                  'Following up on this request - please provide an update on the status.',
                );
                setShowAddNote(true);
              }}>
          ...
          {!showAddNote && (
            <Button variant="outline" onPress={() => setShowAddNote(true)}>
