...
const commonAreaLocations = [
  {
    ...
    areas: ['Main Lobby', 'Staircase', 'Corridors', 'Elevators', 'Entrance Gates', 'Mailbox Area']
  },
  {
    id: 'water-systems',
    title: '🌊 Water Systems',
    icon: <Droplets size={24} color="#6366f1" />,
    areas: ['Water Tank', 'Pump Room', 'Common Taps', 'Overhead Tank', 'Bore Well', 'Water Meter']
  },
  {
    id: 'electrical',
    title: '⚡ Electrical Systems',
    icon: <Zap size={24} color="#6366f1" />,
    areas: ['Common Area Lighting', 'Generator Room', 'Electrical Panel', 'Street Lights', 'Emergency Lighting']
  },
  {
    id: 'parking',
    title: '🚗 Parking & Security',
    icon: <Car size={24} color="#6366f1" />,
    areas: ['Parking Area', 'Security Gates', 'Guard Room', 'Barrier Gates', 'Visitor Parking']
  },
  {
    id: 'garden',
    title: '🌱 Garden & Landscaping',
    icon: <TreePine size={24} color="#6366f1" />,
    areas: ['Garden', 'Lawn Area', 'Plant Beds', 'Trees', 'Irrigation System', 'Pathways']
  },
  {
    id: 'amenities',
    title: '🏃 Amenities',
    icon: <Users size={24} color="#6366f1" />,
    areas: ['Gymnasium', 'Swimming Pool', 'Community Hall', 'Children Play Area', 'Clubhouse']
  },
  {
    id: 'security-systems',
    title: '🔐 Security Systems',
    icon: <Shield size={24} color="#6366f1" />,
    areas: ['CCTV Cameras', 'Intercom System', 'Access Control', 'Security Alarm', 'Fire Safety']
  }
];

// Issue categories
const issueCategories = [
  { id: 'plumbing', title: 'Plumbing', icon: '🔧', description: 'Leakage, Blockage, Pump Issues' },
  { id: 'electrical', title: 'Electrical', icon: '⚡', description: 'Lighting, Power, Wiring Issues' },
  { id: 'civil', title: 'Civil Works', icon: '🏗️', description: 'Cracks, Paint, Flooring' },
  { id: 'cleaning', title: 'Cleaning', icon: '🧹', description: 'Deep cleaning requirements' },
  { id: 'security', title: 'Security', icon: '🔐', description: 'Camera, Gate, Lock issues' },
  { id: 'landscaping', title: 'Landscaping', icon: '🌱', description: 'Garden, Tree, Plant care' },
  { id: 'other', title: 'Other', icon: '📝', description: 'General maintenance issues' }
];
...
export default function CreateCommonAreaRequest() {
  ...
  const selectedLocationData = commonAreaLocations.find(loc => loc.id === selectedLocation);
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className={`text-headline-medium font-semibold ${
                      selectedLocation === location.id ? 'text-primary' : 'text-text-primary'
                    }`}>
                      {location.title}
                    </Text>
                    <Text className="text-body-medium text-text-secondary">
                ...
                {selectedLocationData.areas.map((area) => (
                  ...
                  >
                    <Text className={`text-body-medium font-medium ${
                      selectedArea === area ? 'text-primary' : 'text-text-primary'
                    }`}>
                      {area}
                    </Text>
                  </TouchableOpacity>
            ...
            <View className="flex-row flex-wrap gap-3">
              {issueCategories.map((category) => (
                ...
                >
                  ...
                  <Text className={`text-body-medium font-medium ${
                    selectedCategory === category.id ? 'text-primary' : 'text-text-primary'
                  }`}>
                    {category.title}
                  </Text>
                </TouchableOpacity>
            ...
            <View className="space-y-3">
              {priorityLevels.map((priority) => (
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className={`text-headline-medium font-semibold ${
                      selectedPriority === priority.id ? 'text-primary' : 'text-text-primary'
                    }`}>
                      {priority.title}
                    </Text>
                    <Text className="text-body-medium text-text-secondary">
          ...
          <View>
            ...
            {mediaFiles.length > 0 && (
              <View className="space-y-2">
                {mediaFiles.map((file) => (
                  <View key={file.id} className="flex-row items-center bg-surface rounded-lg p-3 border border-divider">
