...
export default function SocietySelection() {
  ...
  const searchResults = useSocietyOnboardingStore(
    (state) => state.searchResults,
  );
  const searchLoading = useSocietyOnboardingStore(
  ...
  const renderSocietyItem = useCallback(
    ({ item }: { item: SocietyInfo }) => (
      <TouchableOpacity
          ...
          <View className="flex-1">
            ...
            <View className="flex-row items-center">
              <Text className="text-text-secondary text-xs">
                {item.stats?.totalFlats || item.totalFlats || 'N/A'} units •
                Code: {item.code}
              </Text>
            </View>
          </View>
    ...
    <SafeAreaView className="flex-1 bg-background">
      <KeyboardAvoidingView
        ...
        <ResponsiveContainer
          ...
          {searchMode === 'code' ? (
            <View className="mb-6">
              <ValidatedInput
                ...
                onChangeText={(text) =>
                  codeForm.setValue('societyCode', text.toUpperCase())
                }
                ...
                icon={
                  <LucideIcons name="key-round" size={20} color="#6b7280" />
                }
              />
              <Button
          ...
          {searchMode === 'name' && (
            <>
              {searchLoading && (
                <View className="items-center py-8">
                  ...
                  <Text className="text-text-secondary text-xs mt-1">
                    Please wait while we find matching societies
                  </Text>
                </View>
              )}
              ...
              {searchError && (
                <View className="bg-error/5 border border-error/20 rounded-xl p-4 mb-4">
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      const searchValue =
                        nameForm.fields.societyName.value.trim();
                      if (searchValue.length >= 2) {
                        handleNameSearch({ societyName: searchValue });
                      }
                    }}
              ...
              {searchResults && searchResults.length > 0 && (
                <View className="mb-6">
                  <Text className="text-text-primary font-semibold text-lg mb-4">
                    ...
                    {searchTotal && searchTotal > searchResults.length
                      ? ` of ${searchTotal}`
                      : ''}
                    )
                  </Text>
              ...
              {!searchLoading &&
                ...
                nameForm.fields.societyName.value.trim().length >= 2 && (
                  <View className="items-center py-8">
                    ...
                    <Text className="text-text-secondary text-sm text-center px-4">
                      Try different search terms like society name, area, or
                      landmark.{'\n'}Contact your society management if you
                      can't find it.
                    </Text>
                  </View>
                )}
          ...
          {/* Continue Button */}
          {selectedSociety && (
            <Button onPress={handleContinue} className="mb-6">
          ...
          <View className="bg-primary/5 rounded-xl p-4 mt-4">
            ...
            <Text className="text-text-secondary text-sm leading-5">
              ...
              society management for the code{'\n'}• Try searching by society
              name or area name{'\n'}• Check notice boards or society apps for
              this information
            </Text>
          </View>
