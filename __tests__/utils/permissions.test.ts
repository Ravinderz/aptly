...
describe('Permission System', () => {
  ...
  describe('PermissionChecker', () => {
    ...
    describe('hasPermission', () => {
      test('should return true when user has specific permission', () => {
        expect(permissionChecker.hasPermission(mockUser, 'read_posts')).toBe(
          true,
        );
        expect(permissionChecker.hasPermission(mockUser, 'write_posts')).toBe(
          true,
        );
        expect(
          permissionChecker.hasPermission(mockUser, 'manage_residents'),
        ).toBe(true);
      });

      test('should return false when user lacks permission', () => {
        expect(permissionChecker.hasPermission(mockUser, 'delete_posts')).toBe(
          false,
        );
        expect(permissionChecker.hasPermission(mockUser, 'admin_access')).toBe(
          false,
        );
      });

      test('should return false for null or undefined user', () => {
        expect(permissionChecker.hasPermission(null as any, 'read_posts')).toBe(
          false,
        );
        expect(
          permissionChecker.hasPermission(undefined as any, 'read_posts'),
        ).toBe(false);
      });
      ...
      test('should return false when user has no permissions array', () => {
        ...
        expect(
          permissionChecker.hasPermission(userWithoutPermissions, 'read_posts'),
        ).toBe(false);
      });
      ...
      test('should handle permission with resource parameter', () => {
        expect(
          permissionChecker.hasPermission(mockUser, 'read_posts', 'community'),
        ).toBe(true);
        expect(
    ...
    describe('canAccess', () => {
      test('should check access permissions', () => {
        expect(permissionChecker.canAccess(mockUser, 'read', 'posts')).toBe(
          false,
        );
        expect(
          permissionChecker.canAccess(mockUser, 'read_posts', 'community'),
        ).toBe(true);
      });

      test('should return false for unauthorized access', () => {
        expect(
          permissionChecker.canAccess(mockUser, 'delete_all', 'system'),
        ).toBe(false);
      });
    });
    ...
    describe('filterByPermissions', () => {
      ...
      test('should return all items when user has permission', () => {
        const filtered = permissionChecker.filterByPermissions(
          mockUser,
          mockItems,
          'read_posts',
        );
  ...
  describe('Edge Cases', () => {
    test('should handle empty permissions array', () => {
      ...
      expect(
        checker.hasPermission(userWithEmptyPermissions, 'read_posts'),
      ).toBe(false);
    });
    ...
    test('should handle special characters in permissions', () => {
      ...
      expect(
        checker.hasPermission(userWithSpecialPermissions, 'read-posts'),
      ).toBe(true);
      expect(
        checker.hasPermission(userWithSpecialPermissions, 'write.posts'),
      ).toBe(true);
    });
  });
});
