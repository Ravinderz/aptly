...
describe('Picker Configurations', () => {
  ...
  describe('visitorDateConfig', () => {
    ...
    test('should have tomorrow preset with future date', () => {
      ...
      const timeDiff = Math.abs(tomorrowPreset.value.getTime() - tomorrow.getTime());
    ...
    test('should have weekend preset with future date', () => {
      ...
      const today = new Date();
      expect(weekendPreset.value.getTime()).toBeGreaterThanOrEqual(today.getTime());
    });
  });
  ...
  describe('visitorTimeConfig', () => {
    ...
    test('should have all presets within restricted hours', () => {
      visitorTimeConfig.presets.forEach(preset => {
        ...
        expect(preset.value).toMatch(/^(0[6-9]|1[0-2]|0[1-9]):[0-5][0-9] (AM|PM)$/);
      });
    });
  });
  ...
  describe('adminDateConfig', () => {
    ...
    test('should have more presets than visitor config', () => {
      ...
      expect(adminDateConfig.presets.length).toBeGreaterThan(visitorDateConfig.presets.length);
    });
    ...
    test('should have next week preset with future date', () => {
      ...
      const timeDiff = Math.abs(nextWeekPreset.value.getTime() - nextWeek.getTime());
      expect(timeDiff).toBeLessThan(24 * 60 * 60 * 1000); // Within 24 hours
    });
    ...
    test('should have next month preset with future date', () => {
      ...
      const timeDiff = Math.abs(nextMonthPreset.value.getTime() - nextMonth.getTime());
      expect(timeDiff).toBeLessThan(24 * 60 * 60 * 1000); // Within 24 hours
    });
  });
  ...
  describe('adminTimeConfig', () => {
    ...
    test('should have extended time presets', () => {
      ...
      expect(adminTimeConfig.presets.length).toBeGreaterThan(visitorTimeConfig.presets.length);
    });
    ...
    test('should have correct time values for each preset', () => {
      const [earlyMorning, morning, afternoon, evening, lateEvening] = adminTimeConfig.presets;
    ...
    test('should have proper period categorization', () => {
      const morningPresets = adminTimeConfig.presets.filter(p => p.period === 'morning');
      const afternoonPresets = adminTimeConfig.presets.filter(p => p.period === 'afternoon');
      const eveningPresets = adminTimeConfig.presets.filter(p => p.period === 'evening');
  ...
  describe('configuration consistency', () => {
    ...
    test('should have admin config as extension of visitor config', () => {
      expect(adminDateConfig.maxDaysInAdvance).toBeGreaterThan(visitorDateConfig.maxDaysInAdvance);
      expect(adminTimeConfig.presets.length).toBeGreaterThan(visitorTimeConfig.presets.length);
      expect(adminDateConfig.presets.length).toBeGreaterThan(visitorDateConfig.presets.length);
    });
    ...
    test('should have valid time format in all presets', () => {
      ...
      [...visitorTimeConfig.presets, ...adminTimeConfig.presets].forEach(preset => {
        expect(preset.value).toMatch(timeRegex);
      });
    });

    test('should have all date presets as Date objects', () => {
      [...visitorDateConfig.presets, ...adminDateConfig.presets].forEach(preset => {
  ...
  describe('role-based access control', () => {
    test('should restrict visitor time access to business hours', () => {
      const startHour = parseInt(visitorTimeConfig.restrictedHours.start.split(':')[0]);
      const endHour = parseInt(visitorTimeConfig.restrictedHours.end.split(':')[0]);
