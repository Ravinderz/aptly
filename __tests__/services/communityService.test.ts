...
class CommunityService {
  ...
  static async createPost(
    postData: Omit<
      CommunityPost,
      'id' | 'createdAt' | 'updatedAt' | 'likes' | 'comments' | 'isLiked'
    >,
  ...
  static async updatePost(
    postId: string,
    updates: Partial<
      Pick<CommunityPost, 'content' | 'attachments' | 'mentions' | 'tags'>
    >,
  ...
  static async getComments(postId: string): Promise<Comment[]> {
    try {
      ...
      const allComments: Comment[] = commentsJson
  ...
  private static async incrementPostCommentCount(
    postId: string,
  ): Promise<void> {
    const posts = await this.getPosts();
    const postIndex = posts.findIndex((p) => p.id === postId);
  ...
  private static async decrementPostCommentCount(
    postId: string,
  ): Promise<void> {
    const posts = await this.getPosts();
    const postIndex = posts.findIndex((p) => p.id === postId);
  ...
  private static async recordUserInteraction(
    userId: string,
    postId: string,
    action: string,
  ): Promise<void> {
    try {
      const interactionsJson = await AsyncStorage.getItem(
        this.USER_INTERACTIONS_KEY,
      );
...
describe('CommunityService', () => {
  ...
  describe('updatePost', () => {
    ...
    test('should return null for non-existent post', async () => {
      ...
      const result = await CommunityService.updatePost('nonexistent', {
        content: 'Updated',
      });
    ...
    test('should handle update errors', async () => {
      ...
      await expect(
        CommunityService.updatePost('1', { content: 'Updated' }),
  ...
  describe('deletePost', () => {
    ...
    test('should handle delete errors', async () => {
      ...
      mockAsyncStorage.setItem.mockRejectedValue(error);
      mockAsyncStorage.getItem.mockResolvedValue(
        JSON.stringify([{ id: '1', content: 'Test post' }]),
      );

      await expect(CommunityService.deletePost('1')).rejects.toThrow(
        'Delete error',
      );
    });
  });
  ...
  describe('addComment', () => {
    ...
    test('should handle add comment errors', async () => {
      ...
      await expect(CommunityService.addComment(commentData)).rejects.toThrow(
        'Add comment error',
      );
    });
  });
  ...
  describe('searchPosts', () => {
    test('should search posts by content', async () => {
      ...
      expect(result).toHaveLength(2);
      expect(
        result.every((p) => p.content.toLowerCase().includes('react')),
      ).toBe(true);
    });
    ...
    test('should search posts by author name', async () => {
      ...
      expect(result).toHaveLength(2);
      expect(
        result.every((p) => p.authorName.toLowerCase().includes('john')),
      ).toBe(true);
    });
  ...
  describe('Integration Tests', () => {
    test('should handle complete post lifecycle', async () => {
      ...
      // Like post
      const likeResult = await CommunityService.likePost(
        createdPost.id,
        'user2',
      );
