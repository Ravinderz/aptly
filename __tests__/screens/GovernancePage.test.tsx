...
jest.mock('../../contexts/GovernanceContext', () => ({
  useGovernance: () => ({
    ...
    canUserPerformAction: (action: string) => action === 'vote' || action === 'create_campaign',
...
describe('GovernancePage Screen', () => {
  ...
  describe('Initial Rendering', () => {
    it('should render the governance page correctly', () => {
      ...
      expect(screen.getByText('Democratic participation and emergency management')).toBeTruthy();
    });

    it('should show loading state initially', () => {
      // Mock loading state
      jest.mocked(require('../../contexts/GovernanceContext').useGovernance).mockReturnValue({
        state: {
          ...require('../../contexts/GovernanceContext').useGovernance().state,
          isLoading: true
        }
      });
  ...
  describe('Emergency Tab', () => {
    it('should render emergency alerts', () => {
      ...
      expect(screen.getByText('Water Supply Disruption')).toBeTruthy();
      expect(screen.getByText('Water supply disrupted for maintenance')).toBeTruthy();
    });
    ...
    it('should update when new data arrives', () => {
      ...
      jest.mocked(require('../../contexts/GovernanceContext').useGovernance).mockReturnValue({
        ...
        state: {
          ...require('../../contexts/GovernanceContext').useGovernance().state,
          dashboardData: {
            ...require('../../contexts/GovernanceContext').useGovernance().state.dashboardData,
            activeCampaigns: 2
          }
        }
      });
